<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Edit Jawaban untuk Pertanyaan: ') }} {{ Str::limit($question->question_text, 50) }}
        </h2>
    </x-slot>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-8 text-gray-900 dark:text-gray-100">
                    <div class="flex items-center gap-4 mb-8">
                        ...
                        <h3 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Jawaban</h3>
                    </div>
                    <form
                        action="{{ route('admin.modules.quizzes.questions.answers.update', [$module, $quiz, $question, $answer]) }}"
                        method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <x-textarea-input id="answer_text" class="block mt-1 w-full rounded-xl"
                                name="answer_text">{{ old('answer_text', $answer->answer_text) }}</x-textarea-input>
                            ...
                            <label for="is_correct" class="inline-flex items-center cursor-pointer">
                                <div class="relative">
                                    <input id="is_correct" type="checkbox" class="sr-only peer" name="is_correct"
                                        value="1" {{ old('is_correct', $answer->is_correct) ? 'checked' : '' }}>
                                    <div
