            ...
            <div class="bg-white dark:bg-dark-800 overflow-hidden shadow-sm sm:rounded-3xl">
                <div class="p-8 text-gray-900 dark:text-gray-100">
                    <div class="flex items-center gap-4 mb-6">
                        ...
                        <h3 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Konten</h3>
                    </div>
                    ...
                    <form action="{{ route('admin.contents.update', $content) }}" method="POST" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select id="module_id" name="module_id" required
                                    ...
                                    @foreach($modules as $module)
                                        <option value="{{ $module->id }}" {{ old('module_id', $content->module_id) == $module->id ? 'selected' : '' }}>{{ $module->title }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select id="type" name="type" required
                                    class="block w-full mt-1 border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-primary-500 dark:focus:border-primary-600 focus:ring-primary-500 dark:focus:ring-primary-600 rounded-xl shadow-sm">
                                    <option value="article" {{ old('type', $content->type) == 'article' ? 'selected' : '' }}>Artikel</option>
                                    ...
                                    <option value="quiz" {{ old('type', $content->type) == 'quiz' ? 'selected' : '' }}>
                                        Kuis</option>
                                </select>
                        ...
                        <div x-data="{ type: '{{ old('type', $content->type) }}' }"
                            ...
                            <template x-if="type === 'article'">
                                <div>
                                    ...
                                    <x-textarea-input id="body" class="block mt-1 w-full rounded-xl" name="body"
                                        rows="10">{{ old('body', $content->body) }}</x-textarea-input>
                            ...
                            <template x-if="type === 'video' || type === 'infographic'">
                                <div>
                                    ...
                                    <x-text-input id="media_url" class="block mt-1 w-full rounded-xl" type="text"
                                        name="media_url" :value="old('media_url', $content->media_url)" />
                                ...
                                <div>
                                    ...
                                    <select id="quiz_id" name="quiz_id" :required="type === 'quiz'"
                                        ...
                                        @foreach($quizzes as $quiz)
                                            <option value="{{ $quiz->id }}" {{ old('quiz_id', $content->quiz_id) == $quiz->id ? 'selected' : '' }}>
