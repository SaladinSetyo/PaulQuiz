<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Edit Pertanyaan untuk Kuis: ') }} {{ $quiz->title }}
        </h2>
    </x-slot>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-8 text-gray-900 dark:text-gray-100">
                    <div class="flex items-center gap-4 mb-8">
                        ...
                        <h3 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Pertanyaan</h3>
                    </div>
                    <form action="{{ route('admin.modules.quizzes.questions.update', [$module, $quiz, $question]) }}"
                        method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-6">
                            ...
                            <x-textarea-input id="question_text" class="block mt-1 w-full rounded-xl"
                                name="question_text"
                                required>{{ old('question_text', $question->question_text) }}</x-textarea-input>
                        ...
                        <div class="mb-6">
                            ...
                            <x-textarea-input id="description" class="block mt-1 w-full rounded-xl"
                                name="description">{{ old('description', $question->description) }}</x-textarea-input>
                        ...
                        <div class="mt-12" x-data="{ 
                            answers: {{ json_encode($question->answers->map(fn($a) => ['text' => $a->answer_text, 'is_correct' => $a->is_correct])) }},
                            addAnswer() {
