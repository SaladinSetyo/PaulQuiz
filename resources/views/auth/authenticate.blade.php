...
<body class="font-sans antialiased bg-gray-100 dark:bg-dark-900 text-gray-900 dark:text-white">
    ...
    <style>
        ...
        @media (min-width: 768px) { .auth-slider { display: flex; } }
        .no-scrollbar::-webkit-scrollbar { display: none; }
        .no-scrollbar { -ms-overflow-style: none; scrollbar-width: none; }
        
        .form-container { opacity: 0; pointer-events: none; transition: all 0.7s ease-in-out; }
        .form-container.active { opacity: 1; pointer-events: auto; }

        /* FORCE ANIMATION SPEED */
        #auth-slider-overlay {
            transition: all 5s cubic-bezier(0.85, 0, 0.15, 1) !important;
        }
    ...
    <div x-data="{ 
        ...
        <div class="absolute inset-0 overflow-hidden pointer-events-none">
            <div class="absolute -top-[20%] -left-[10%] w-[70vw] h-[70vw] bg-indigo-900/10 rounded-full blur-[120px]"></div>
            <div class="absolute top-[30%] -right-[20%] w-[60vw] h-[60vw] bg-blue-900/10 rounded-full blur-[120px]"></div>
        </div>
                ...
                <div class="w-full max-w-sm mx-auto space-y-4">
                    ...
                    <form method="POST" action="{{ route('login') }}" class="space-y-3">
                        ...
                        <div class="space-y-1">
                            ...
                            <input class="custom-input block w-full px-5 py-3.5 rounded-xl" 
                        ...
                        <div class="flex items-center justify-between">
                            <label class="inline-flex items-center cursor-pointer group">
                                <div class="relative flex items-center">
                                    ...
                                    <div class="w-5 h-5 border-2 border-slate-700 rounded-md peer-checked:bg-indigo-600 peer-checked:border-indigo-600 transition-all"></div>
                            ...
                            @if (Route::has('password.request'))
                                <a class="text-sm text-indigo-400 hover:text-indigo-300 font-bold transition-colors" href="{{ route('password.request') }}">Lupa Password?</a>
                            @endif
                        </div>

                        <button type="submit" class="w-full py-3.5 bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-500 hover:to-blue-500 text-white font-bold rounded-xl shadow-lg shadow-indigo-900/50 hover:shadow-indigo-600/50 transition-all transform hover:-translate-y-0.5">
                            Masuk Sekarang
                        </button>
                    </form>
                    ...
                    <div class="text-center pt-2">
                        <p class="text-sm text-slate-500">
                            Belum punya akun? 
                            <button @click="toggle" class="text-indigo-400 font-bold hover:underline hover:text-indigo-300 transition-colors">Daftar Gratis</button>
                        </p>
                    </div>
                </div>
            </div>
                ...
                <div class="w-full max-w-sm mx-auto space-y-4">
                    ...
                    <form method="POST" action="{{ route('register') }}" class="space-y-3">
                        ...
                        <div class="space-y-1">
                            ...
                            <input class="custom-input block w-full px-5 py-3 rounded-xl" 
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <div class="space-y-1">
                                <label class="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1">Konfirmasi</label>
                                <input class="custom-input block w-full px-5 py-3 rounded-xl" 
                                       type="password" name="password_confirmation" required />
                            </div>
                        </div>

                        <button type="submit" class="w-full py-3.5 bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-500 hover:to-blue-500 text-white font-bold rounded-xl shadow-lg shadow-indigo-900/50 hover:shadow-indigo-600/50 transition-all transform hover:-translate-y-0.5 mt-2">
                            Daftar Sekarang
                        </button>
                    </form>
                    ...
                    <div class="text-center pt-2">
                        <p class="text-sm text-slate-500">
                            Sudah punya akun? 
                            <button @click="toggle" class="text-indigo-400 font-bold hover:underline hover:text-indigo-300 transition-colors">Masuk Saja</button>
                        </p>
                    </div>
                </div>
            </div>

            <!-- Slider Overlay - ID #auth-slider-overlay FORCED SPEED -->
            <div id="auth-slider-overlay" class="auth-slider absolute top-0 h-full z-20 items-center justify-center overflow-hidden"
                ...
                <div class="absolute inset-0 pointer-events-none">
                    <div class="absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-20 brightness-150 mix-blend-overlay"></div>
                    <!-- Vivid Blobs -->
                    <div class="absolute top-[-50px] left-[-50px] w-64 h-64 bg-indigo-500 rounded-full blur-[80px] opacity-40 animate-pulse"></div>
                    <div class="absolute bottom-[-50px] right-[-50px] w-64 h-64 bg-blue-500 rounded-full blur-[80px] opacity-40"></div>
                </div>

                <div class="relative z-10 text-center p-12 text-white max-w-md">
                    <div class="mb-6 transform transition-transform duration-500 hover:scale-105 drop-shadow-2xl flex justify-center">
                        <x-application-logo class="h-80 w-auto fill-current text-white filter drop-shadow-[0_0_25px_rgba(255,255,255,0.4)]" />
                    </div>
                    ...
                    <button @click="toggle" class="px-8 py-3.5 bg-white/10 hover:bg-white/20 border border-white/20 backdrop-blur-md rounded-full font-bold transition-all shadow-lg hover:shadow-white/20 hover:-translate-y-1">
