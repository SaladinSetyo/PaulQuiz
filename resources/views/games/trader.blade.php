...
<body class="flex flex-col h-screen bg-[#0b0e11] text-[#eaecef] select-none text-xs overflow-hidden">
    ...
    <header class="h-12 bg-[#181a20] border-b border-[#2b3139] flex items-center px-4 shrink-0 justify-between z-50">
        <div class="flex items-center gap-4">
            <div class="flex items-center gap-2">
                <div class="w-6 h-6 rounded bg-[#f0b90b] text-[#0b0e11] font-black flex items-center justify-center text-[11px]">C</div>
                <span class="font-bold text-sm text-white">BTCUSDT</span>
                <span class="text-[10px] bg-[#2b3139] px-2 py-0.5 rounded text-emerald-400 font-bold border border-emerald-500/20">Perp</span>
            </div>
            
            <div class="hidden md:flex items-center gap-6 border-l border-[#2b3139] pl-6 h-full">
                 <div class="flex flex-col justify-center h-full">
                    <span class="text-[12px] font-bold text-emerald-400" x-data x-text="window.game?.lastPrice?.toFixed(2) ?? '---'"></span>
        ...
        <div class="flex items-center gap-4">
             <div class="flex items-center gap-2 bg-[#2b3139] px-3 py-1.5 rounded-md" x-data>
                 <span class="w-2 h-2 rounded-full" :class="window.game?.phase === 'open' ? 'bg-emerald-500 animate-pulse' : 'bg-amber-500'"></span>
                 <span class="font-bold text-white uppercase text-[10px] tracking-wide" x-text="window.game?.phase === 'open' ? 'TRADING' : 'LOCKED'"></span>
                 <span class="font-mono font-bold text-[#f0b90b] text-sm ml-2" x-text="window.game?.formatTimer() ?? '00:00'"></span>
             </div>
             
             <div class="flex flex-col items-end leading-none">
                  <span class="text-[10px] text-slate-500 font-bold uppercase mb-0.5">Wallet</span>
                  <span class="font-mono font-bold text-white text-[13px]" x-text="'$' + (window.userBalance?.toLocaleString('en-US') ?? '1,000')"></span>
             </div>
             
             <a href="{{ route('homepage') }}" class="flex items-center justify-center w-8 h-8 rounded hover:bg-[#2b3139] text-slate-400 hover:text-white transition-colors">
    ...
    <!-- MAIN GRID LAYOUT -->
    <div x-data="proTrader()" x-init="initTrader()" x-cloak class="flex-grow grid grid-cols-[1fr_280px_300px] overflow-hidden bg-[#0b0e11]">
        
        <!-- COL 1: CHART (Main) -->
        <div class="flex flex-col min-w-0 min-h-0 bg-[#0b0e11] relative border-r border-[#2b3139]">
            <!-- Chart Toolbar -->
            <div class="h-9 border-b border-[#2b3139] flex items-center px-4 gap-4 text-[11px] font-bold text-slate-500 bg-[#0b0e11]">
            ...
            <div class="h-[200px] border-t border-[#2b3139] bg-[#0b0e11] flex flex-col shrink-0">
                <div class="h-9 flex items-center px-4 gap-6 border-b border-[#2b3139] bg-[#14161b]">
                    <span class="text-[11px] font-bold text-[#f0b90b] border-b-2 border-[#f0b90b] h-full flex items-center px-1 cursor-pointer">Positions <span x-text="myPosition ? '(1)' : '(0)'"></span></span>
                    <span class="text-[11px] font-bold text-slate-500 h-full flex items-center px-1 cursor-pointer hover:text-white">Open Orders (0)</span>
                    <span class="text-[11px] font-bold text-slate-500 h-full flex items-center px-1 cursor-pointer hover:text-white">Order History</span>
                </div>
                ...
                <div class="flex-grow overflow-auto custom-scrollbar relative bg-[#0b0e11]">
                    <table class="w-full text-left font-mono text-[11px]" x-show="myPosition">
                        ...
                        <tbody class="text-white">
                            <tr class="bg-[#1e2329]/20 border-b border-[#2b3139] hover:bg-[#1e2329]/40 transition-colors">
                                ...
                                <td class="px-4 py-2 text-right text-slate-300" x-text="myPosition?.entry.toFixed(2)"></td>
                    ...
                    <div x-show="!myPosition" class="absolute inset-0 flex flex-col items-center justify-center text-slate-600">
        ...
        <!-- COL 2: ORDER BOOK (Fixed 280px) -->
        <div class="flex flex-col border-r border-[#2b3139] bg-[#14161b] min-w-0">
            ...
            <div class="flex-grow overflow-hidden flex flex-col font-mono text-[10px] relative">
                ...
                <div class="flex-1 overflow-hidden flex flex-col-reverse justify-start">
                    <template x-for="ask in asks" :key="ask.id">
                        <div class="flex justify-between px-3 py-[1px] relative hover:bg-[#2b3139] cursor-pointer group h-[18px] items-center">
                ...
                <!-- Ticker -->
                <div class="h-10 flex items-center justify-center border-y border-[#2b3139] bg-[#0b0e11] shrink-0 my-1">
                    ...
                    <span class="text-[10px] font-bold text-slate-500 ml-2">Mark</span>
                </div>
        ...
        <div class="flex flex-col bg-[#1e2329] min-w-0 border-l border-[#2b3139]">
            ...
            <div class="p-4 flex flex-col gap-5 overflow-y-auto">
                 <div class="flex bg-[#2b3139] rounded p-[2px] shrink-0">
                    <button class="flex-1 py-1.5 rounded text-[10px] font-bold bg-[#474d57] text-white shadow-sm">Limit</button>
                    <button class="flex-1 py-1.5 rounded text-[10px] font-bold text-slate-400 hover:text-white transition-colors">Market</button>
                    <button class="flex-1 py-1.5 rounded text-[10px] font-bold text-slate-400 hover:text-white transition-colors">Stop</button>
                </div>
                ...
                <div class="space-y-4">
                     ...
                     <!-- Price Input -->
                    <div class="flex items-center bg-[#2b3139] rounded border border-[#2b3139] h-10 hover:border-[#f0b90b] transition-colors group">
                        <span class="pl-3 text-[11px] font-bold text-slate-400 w-16 group-hover:text-slate-300">Price</span>
                        <input type="text" disabled value="Market Price" class="flex-grow bg-transparent text-right pr-3 text-xs font-bold text-white outline-none cursor-not-allowed opacity-70">
                        <span class="pr-3 text-[11px] font-bold text-slate-500">USDT</span>
                    </div>
                    
                    <!-- Amount Input -->
                    <div class="flex items-center bg-[#2b3139] rounded border border-[#2b3139] h-10 hover:border-[#f0b90b] transition-colors group focus-within:border-[#f0b90b]">
                        <span class="pl-3 text-[11px] font-bold text-slate-400 w-16 group-hover:text-slate-300 group-focus-within:text-[#f0b90b]">Amount</span>
        ...
        <!-- RESULT POPUP -->
         <div x-show="showResult" style="display: none;" 
            ...
            <div class="bg-[#1e2329] p-8 rounded-2xl border border-[#474d57] text-center shadow-2xl min-w-[320px] transform hover:scale-105 transition-transform duration-300">
                <div class="text-6xl mb-4" x-text="lastWin ? '💰' : '💸'"></div>
                <h2 class="text-3xl font-black text-white mb-1 uppercase tracking-tighter" x-text="lastWin ? 'Take Profit' : 'Stop Loss'"></h2>
                ...
                <div class="bg-[#0b0e11] p-4 rounded-xl border border-[#2b3139]">
                    ...
                    <div class="flex justify-between items-end">
                        ...
                        <div class="text-[10px] text-slate-500 font-mono" x-text="new Date().toLocaleTimeString()"></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            ...
            Alpine.data('proTrader', () => ({
                ...
                canvas: null, ctx: null, maxCandles: 50, resizeObserver: null,
