...
<body class="flex flex-col h-screen bg-[#0b0e11] text-[#eaecef] select-none text-xs overflow-hidden">
    ...
    <div x-data="proTrader()" x-init="initTrader()" x-cloak
        ...
        <div class="flex flex-col min-w-0 min-h-[400px] lg:min-h-0 bg-[#0b0e11] relative border-r border-[#2b3139] overflow-hidden"
            ...
            <div @click="switchTab('positions')"
                :class="activeTab === 'positions' ? 'text-white border-[#f0b90b]' : 'text-slate-500'"
                class="flex-1 px-4 py-3 text-[11px] font-bold border-b-2 transition-all cursor-pointer hover:text-white">
                <!-- Bottom Tabs: Positions & History -->
                <div class="border-t border-[#2b3139] bg-[#0b0e11] flex flex-col shrink-0"
                    style="height: min(300px, calc(100vh - 500px)); min-height: 180px;"
                    x-data="{ activeTab: 'positions' }">
                    <div class="h-9 flex items-center px-4 gap-6 border-b border-[#2b3139] bg-[#14161b]">
                        <span @click="activeTab = 'positions'" <div @click="switchTab('orders')"
                            :class="activeTab === 'orders' ? 'text-white border-[#f0b90b]' : 'text-slate-500'"
                            class="flex-1 px-4 py-3 text-[11px] font-bold border-b-2 transition-all cursor-pointer hover:text-white">
                            class="text-[11px] font-bold h-full flex items-center px-1 cursor-pointer
                            smooth-transition-fast">Positions
                        ...
                        <span @click="activeTab = 'history'"
                            ...
                            <div @click="switchTab('history')"
                            :class="activeTab === 'history' ? 'text-white border-[#f0b90b]' : 'text-slate-500'"
                            class="flex-1 px-4 py-3 text-[11px] font-bold border-b-2 transition-all cursor-pointer hover:text-white">
                            History
                            ...
                            <table class="w-full text-left font-mono text-[11px]" x-show="myPosition">
                                ...
                                <tbody class="text-white">
                                    <tr
                                        ...
                                        <td class="px-4 py-2 text-right text-slate-300" x-text="lastPrice.toFixed(2)">
                                        </td>
                                ...
                                <tbody class="text-white">
                                    <template x-for="trade in tradeHistory" :key="trade.timestamp">
                                        <tr class="border-b border-[#2b3139] hover:bg-[#1e2329]/20 transition-colors">
                                            ...
                                            <td class="px-4 py-2 text-right text-slate-300"
                                                x-text="trade.entry.toFixed(2)">
                                            </td>
                                            <td class="px-4 py-2 text-right text-slate-300"
                                                x-text="trade.exit.toFixed(2)">
                                            </td>
            ...
            <div class="flex flex-col bg-[#1e2329] min-w-0 border-l border-[#2b3139]">
                ...
                <div class="flex bg-[#181a20] text-[11px] font-bold border-b border-[#2b3139] shrink-0">
                    <button @click="setLeverage('spot')"
                        :class="activeLeverage === 'spot' ? 'text-[#f0b90b] border-[#f0b90b]' : 'text-slate-500'"
                        class="flex-1 py-3 border-t-2 bg-[#1e2329] transition-colors">Spot</button>
                    <button @click="setLeverage('cross')"
                        :class="activeLeverage === 'cross' ? 'text-[#f0b90b] border-[#f0b90b]' : 'text-slate-500'"
                        class="flex-1 py-3 transition-colors">Cross 3x</button>
                    <button @click="setLeverage('iso')"
                        :class="activeLeverage === 'iso' ? 'text-[#f0b90b] border-[#f0b90b]' : 'text-slate-500'"
                ...
                <div class="p-4 flex flex-col gap-5 overflow-y-auto">
                    <div class="flex bg-[#2b3139] rounded p-[2px] shrink-0">
                        <button @click="setOrderType('market')"
                            :class="orderType === 'market' ? 'bg-[#2b3139] text-white' : 'text-slate-500'"
                            class="flex-1 py-2 text-[11px] font-bold transition-colors">Market</button>
                        <button @click="setOrderType('limit')"
                            :class="orderType === 'limit' ? 'bg-[#2b3139] text-white' : 'text-slate-500'"
                            class="flex-1 py-2 text-[11px] font-bold transition-colors">Limit</button>
                        <button @click="setOrderType('stop')"
                            :class="orderType === 'stop' ? 'bg-[#2b3139] text-white' : 'text-slate-500'"
                            class="flex-1 py-2 text-[11px] font-bold transition-colors">Stop</button>
                        class="flex-1 py-1.5 rounded text-[10px] font-bold text-slate-400 hover:text-white
                        transition-colors">Market</button>
                        <button
                        ...
                        <div class="p-6">
                            ...
                            <div x-show="tutorialStep === 0" class="space-y-4">
                                <div class="text-center mb-6">
                                    ...
                                    <p class="text-slate-400">Di Crypto Trading Panic - Game survival trading yang
                                        edukatif
                                    </p>
                                </div>
                                    ...
                                    <div class="flex items-start gap-3">
                                        ...
                                        <div>
                                            ...
                                            <ul class="text-sm text-slate-400 mt-1 space-y-1">
                                                ...
                                                <li>â€¢ Balance $0 = <span class="text-[#f6465d] font-bold">GAME
                                                        OVER</span>
                                                </li>
                                            </ul>
                                        </div>
                            ...
                            <div x-show="tutorialStep === 1" class="space-y-4">
                                ...
                                <div class="text-sm text-slate-300 space-y-2">
                                    <p><span class="text-emerald-400 font-bold">â–  Candle Hijau</span> = Harga NAIK
                                        (Close >
                                        Open)</p>
                                    <p><span class="text-[#f6465d] font-bold">â–  Candle Merah</span> = Harga TURUN
                                        (Close < Open)</p>
                                            <p class="mt-3 text-slate-500">ðŸ’¡ <strong>Tips:</strong> Perhatikan pola
                                                candle
                            ...
                            <div x-show="tutorialStep === 4" class="space-y-4">
                                ...
                                <div class="bg-[#0b0e11] p-3 rounded-lg text-xs font-mono space-y-1">
                                    ...
                                    <div
                                        class="border-t border-b border-[#2b3139] py-2 text-center text-white font-bold">
                                        65,100.00</div>
                                ...
                                <div class="text-sm text-slate-300 space-y-2">
                                    ...
                                    <p class="text-slate-500 text-xs mt-3">ðŸ’¡ Spread kecil = Likuiditas tinggi = Lebih
                                        mudah
                            ...
                            <div x-show="tutorialStep === 5" class="space-y-4">
                                ...
                                <div class="bg-[#0b0e11] p-4 rounded-lg border border-[#f0b90b]/30">
                                    <p class="text-xs text-slate-400 mb-2">ðŸŽ¯ <strong
                                            class="text-white">Goal:</strong></p>
                                    <p class="text-sm text-slate-300">Profit untuk bertahan hidup & capai high score!
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    Alpine.data('proTrader', () => ({
                        ...
                        placeOrder(type) {
                            ...
                            if (this.balance < this.betAmount) {
                                alert('Insufficient balance! Need $' + this.betAmount + ', have $' + Math.floor(this.balance));
                                return;
                            }
                            if (this.myPosition) {
                            ...
                            this.myPosition = {
                                ...
                                leverage: this.getCurrentLeverageMultiplier(),
                                timestamp: Date.now()
                            };

                            // Lock phase for countdown
                            this.phase = 'locked';
                            this.timer = 10;  // 10 second countdown

                            // Save state
                            this.saveGame();

                            console.log('Position opened:', this.myPosition);
                        },
                        ...
                        <template x-for="(leader, index) in leaders" :key="leader.id">
                            <div class="bg-[#0b0e11] p-3 rounded-lg border border-[#2b3139]">
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center gap-2">
                                        <span class="text-[#f0b90b] font-bold text-lg"
                                            x-text="'#' + (index + 1)"></span>
