...
<body
    ...
    <div x-data="binaryGame()" x-init="document.addEventListener('alpine:init', () => initGame())" x-cloak
        ...
        <div class="w-full max-w-5xl flex justify-between items-end mb-6 z-10">
            <div>
                <h1
                    class="text-4xl font-extrabold tracking-tight mb-1 text-emerald-400 neon-text-glow">
                    BINARY TRADER</h1>
        ...
        <div
            ...
            <div class="relative h-[400px] w-full bg-slate-900 rounded-[20px] overflow-hidden">
                ...
                <div class="absolute top-4 left-6 z-20">
                    <div x-show="phase === 'betting'" class="bg-cyan-900/80 backdrop-blur border border-cyan-500/50 px-4 py-2 rounded-xl animate-pulse">
                    ...
                    <div x-show="phase === 'trading'" class="bg-slate-900/80 backdrop-blur border border-slate-500/50 px-4 py-2 rounded-xl">
                        <span class="text-slate-400 font-bold uppercase text-xs tracking-wider">Position</span>
                        <div class="text-xl font-black flex items-center gap-2" :class="prediction === 'up' ? 'text-emerald-400' : 'text-rose-400'">
                            <span x-text="prediction === 'up' ? 'LONG (▲)' : 'SHORT (▼)'"></span>
                            <span class="text-sm text-white opacity-60">@ $<span x-text="entryPrice.toFixed(2)"></span></span>
                        </div>
                    </div>
                </div>
                ...
                <div x-show="phase === 'result'"
                    style="display: none;"
                    class="absolute inset-0 z-40 flex flex-col items-center justify-center bg-slate-900/95 backdrop-blur-md transition-all">
                    <div class="text-center transform transition-all" x-bind:class="roundWon ? 'scale-100' : 'scale-100'">
                        <div class="text-6xl mb-4" x-text="roundWon ? '🤑' : '💸'"></div>
                        <h2 class="text-5xl font-black mb-2" :class="roundWon ? 'text-emerald-400' : 'text-rose-400'" x-text="roundWon ? 'PROFIT!' : 'LOSS'"></h2>
                        <p class="text-2xl text-white font-mono font-bold mb-8">
                            <span x-text="roundWon ? '+' : '-'"></span>$<span x-text="formatMoney(roundWon ? betAmount * 0.8 : betAmount)"></span>
                        </p>
                        <button @click="startRound()"
                ...
                <div x-show="phase === 'gameover'"
                    ...
                    <p class="text-xl text-slate-400 mb-8 max-w-md text-center">You blew the entire account. The market waits for no one.</p>
            ...
            <div class="bg-slate-800 p-2">
                ...
                <div x-show="phase === 'betting'" class="grid grid-cols-2 gap-2 h-32">
                    <button @click="placeBet('up')" 
                        ...
                        <span class="text-3xl font-black text-emerald-950 uppercase relative z-10 flex items-center gap-2">
                            PREDICT UP <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 10l7-7m0 0l7 7m-7-7v18"></path></svg>
                        </span>
                    ...
                    <button @click="placeBet('down')" 
                        ...
                        <span class="text-3xl font-black text-rose-950 uppercase relative z-10 flex items-center gap-2">
                            PREDICT DOWN <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M19 14l-7 7m0 0l-7-7m7 7V3"></path></svg>
                        </span>
                ...
                <div x-show="phase === 'trading'" class="h-32 bg-slate-900 rounded-xl flex items-center justify-around border border-slate-700 relative overflow-hidden">
                     <div class="absolute inset-x-0 bottom-0 h-1 bg-slate-800">
                        <div class="h-full bg-blue-500 transition-all duration-1000 ease-linear" :style="'width: ' + (timer / 30 * 100) + '%'"></div>
                     </div>
                     
                     <div class="text-center">
                        <div class="text-xs text-slate-500 uppercase font-bold mb-1">Entry Price</div>
                        <div class="text-2xl font-mono font-bold text-white">$<span x-text="entryPrice.toFixed(2)"></span></div>
                     </div>
                     ...
                     <div class="text-center px-8 py-2 rounded-lg"
                        :class="(prediction === 'up' && price > entryPrice) || (prediction === 'down' && price < entryPrice) ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-rose-500/20 text-rose-400 border border-rose-500/30'">
                         ...
                         <div class="text-3xl font-black" x-text="(prediction === 'up' && price > entryPrice) || (prediction === 'down' && price < entryPrice) ? 'WINNING' : 'LOSING'"></div>
                     </div>
        ...
        <div class="w-full max-w-5xl mt-6">
            ...
            <div class="h-4 bg-slate-800 rounded-full overflow-hidden border border-slate-700">
                <!-- Betting Phase Bar -->
                <div x-show="phase === 'betting'" class="h-full bg-cyan-500 transition-all duration-1000 ease-linear" :style="'width: ' + (timer / 15 * 100) + '%'"></div>
                <!-- Trading Phase Bar (Hidden handled in inner div above but kept here for consistecy if needed) -->
                 <div x-show="phase === 'trading'" class="h-full bg-slate-700 w-full relative">
                    <div class="absolute inset-y-0 left-0 bg-blue-500 transition-all duration-1000 ease-linear" :style="'width: ' + (timer / 30 * 100) + '%'"></div>
                 </div>
            </div>
        </div>
